...
//         </motion.section>
        ...
        //         viewport={{ once: true }}
        //         transition={{ duration: 0.5, delay: index * 0.1 }}
        //         className="group relative bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl overflow-hidden hover:border-emerald-300/30 transition-all duration-500"
        //       >
        //         {/* Book cover */}
        ...
        //         {/* Content */}
        //         <div className="p-4">
        //           <h3 className="font-bold text-white mb-1 group-hover:text-emerald-300 transition-colors duration-300 line-clamp-1">
        //             {book.title}
        //           </h3>
          ...
          <div className="flex justify-center mt-8 gap-2">
            {passions.map((_, index) => (
              <button
                key={index}
                onClick={() => setActiveIndex(index)}
                className={`w-3 h-3 rounded-full transition-all ${index === activeIndex ? 'bg-emerald-400 w-6' : 'bg-gray-600'}`}
              />
            ))}
          </div>
        </motion.section>
